...
export const signIn = async (req, res) => {
  try {
    ...
    res.cookie("refreshToken", refreshToken, {
      httpOnly: true,
      secure: isProd,
      sameSite: isProd ? "none" : "lax", // allow local dev without HTTPS
      maxAge: REFRESH_TOKEN_TTL,
      path: "/", // make cookie available site-wide (important for clearing and requests)
    });
...
export const signOut = async (req, res) => {
  try {
    ...
    if (token) {
      ...
      // xoá cookie using same options used when setting it so browsers remove it reliably
      const isProd = process.env.NODE_ENV === "production";
